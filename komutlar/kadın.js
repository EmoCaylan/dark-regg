...
let kadınrol = message.guild.roles.cache.find(r => r.id === ayarlar.kadınrol)
let kadınrol2 = message.guild.roles.cache.find(r => r.id === ayarlar.kadınrol2)
...
return message.reply(`Bu işlemi sadece yetkililer kullanabilir!!!`).then(x => x.delete({timeout:5000}))
...
if(!args[0]) return message.channel.send('Bir kullanıcı belirt').then(x => x.delete({timeout:5000}))
...
if(!name) return message.channel.send('Bir isim belirt.').then(x => x.delete({timeout:5000}))
if(!age) return message.channel.send('Bir yaş belirt.').then(x => x.delete({timeout:5000}))
...
message.react('<a:pembetik:832366829695270912>')
 ...
 const a = await datab.fetch(`Eskiİsimler.${message.guild.id}.${member.id}`);
  ...
  let uzunluk;
  if (fdh.length >= 6) uzunluk = 5
  if (fdh.length < 6) uzunluk = fdh.length
  ...
  const kanal = member.guild.channels.cache.find(r => r.id === "832366079178440718");
  kanal.send(new Discord.MessageEmbed()
.setDescription(`
 <:kadn:834190388175831061> ${member} adlı üye sunucumuza **kadın** olarak kaydedilmiştir.

 <a:nlem:834190310154305567>__**Üyenin daha önceki kayıtlı isimleri;**__ \`(SON 10)/``  \n\n${data.join('\n')} `).setColor('0x348f36').setFooter(message.author.tag, message.author.avatarURL({dynamic: true})).setTimestamp()  
